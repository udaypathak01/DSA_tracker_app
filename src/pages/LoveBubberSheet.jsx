...
/**
 * Love Bubber's DSA Sheet Main Page
 * Independent DSA sheet with dedicated analytics and tracking
 */
function LoveBubberSheet() {
  const {
    problems,
    loading,
    analytics,
  ...
  const handleReset = async () => {
    if (window.confirm('Are you sure? This will reset all your Love Bubber progress.')) {
      setIsResetting(true);
      try {
        await resetProgress();
      } finally {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="space-y-4">
        <div>
          ...
          <p className="text-xs sm:text-sm md:text-base text-slate-600 dark:text-slate-400 mt-1">
            Structured DSA Roadmap with Dedicated Analytics
          </p>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants}>
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-dark-card dark:to-dark-border rounded-lg p-3 sm:p-4 border border-green-200 dark:border-green-900">
...
export default LoveBubberSheet;
