...
import useStriverA2Z from '../hooks/useStriverA2Z';
...
/**
 * Striver's A2Z Sheet - Learn DSA from A to Z Main Page
 * Independent DSA curriculum with dedicated analytics and tracking
 */
function StriverA2ZSheet() {
  const {
    problems,
    loading,
    analytics,
  ...
  const handleReset = async () => {
    if (window.confirm('Are you sure? This will reset all your A2Z progress.')) {
      setIsResetting(true);
      try {
        await resetProgress();
      } finally {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="space-y-4">
        <div>
          ...
          <p className="text-xs sm:text-sm md:text-base text-slate-600 dark:text-slate-400 mt-1">
            Learn DSA from A to Z with Structured Curriculum
          </p>
        </div>
      </motion.div>
      ...
      <motion.div variants={itemVariants}>
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 dark:from-dark-card dark:to-dark-border rounded-lg p-3 sm:p-4 border border-purple-200 dark:border-purple-900">
...
export default StriverA2ZSheet;
